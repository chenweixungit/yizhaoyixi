      ...
      <el-tabs type="border-card" :value="IASelection">
        ...
        <el-tab-pane label="转库申请单" name="转库申请单">
          <div style="height: 650px">
            <block><el-button type="success" id="create-list" @click="toCreateCenterCreateShiftReq">新建转库申请单</el-button></block>
            <block>
        ...
        <el-tab-pane label="库存盘点" name="库存盘点">
          <div style="height: 650px">
            <block><el-button type="success" id="create-list" @click="toCenterCreateInventoryCheck">新建库存盘点单</el-button></block>
            <block>
        ...
        <el-tab-pane label="期初库存" name="期初库存">
          <div style="height: 650px">
            <block><el-button type="success" id="create-list" @click="toCenterCreateInitialInventory">新建期初库存</el-button></block>
            <block>
...
export default {
  ...
  methods: {
    ...
    toCreateCenterCreateShiftReq () {
      ...
      response.then(function (e) {
        if (e.data.code === 0) {
          _this.$router.push('/CenterCreateShiftReq')
    ...
    toCenterCreateInventoryCheck () {
      ...
      response.then(function (e) {
        if (e.data.code === 0) {
          _this.$router.push('/CenterCreateInventoryCheck')
          _this.$store.dispatch('setCurrentInventoryCheck', {checkId: e.data.data})
        }
      })
    },
    toCenterCreateInitialInventory () {
      this.$router.push('/CenterCreateInitialInventory')
    },
    // 页码跳转
    ...
    editInit (scope) {
      ...
      response.then(function (e) {
        ...
        if (store.state.initDetail !== null) {
          _this.$router.push('/CenterCreateInitialInventory')
        }
      })
    }
  }
