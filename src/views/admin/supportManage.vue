  ...
  <el-container>
    ...
    <el-main class="main_container">
      <el-row type="flex" justify="start">
        ...
        <el-col :span="1"><el-button type="success" size="medium" @click="addSupplier">添加</el-button></el-col>
      </el-row>
    </el-main>
    <card>
      <div class="container-table">
        <el-table :data="tableData.slice((currentPage-1)*pagesize,currentPage*pagesize)"
          ...
          <el-table-column prop="code" label="供应商编码" width="150px">
          </el-table-column>
          <el-table-column prop="name" label="供应商名称" width="250px">
          </el-table-column>
          <el-table-column prop="type" label="供应商类型" width="150px">
          </el-table-column>
...
export default {
  beforeMount () {
    const response = findAllSupplier()
    let _this = this
  ...
  methods: {
    ...
    handleStart (index, row) { // 启用
      const response = enableSupplier({id: row.id})
      let _this = this
      response.then(function (e) {
        console.log(e.data)
        if (e.data.code === 0) {
    ...
    handleEnd (index, row) { // 停用
      const response = stopSupplier({id: row.id})
      let _this = this
      response.then(function (e) {
        console.log(e.data)
        if (e.data.code === 0) {
    ...
    handleDelete (index, row) {
      const response = deleteSupplier({id: row.id})
      let _this = this
      response.then(function (e) {
        console.log(e.data)
        if (e.data.code === 0) {
    ...
    onSubmit (done) { // 表单提交函数
      ...
      if (this.currentSupplierStatus === 0) {
        console.log(this.form)
        const response = insertSupplier({
          name: this.form.name,
          type: this.form.type,
        ...
        const updateResponse = updateSupplier(this.form)
